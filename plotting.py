...
df_sorted = res_df.sort_values('reward')
...
for x in xs:
    idx = df1[df1['alpha'] == x]['reward'].idxmin()
    y1.append(df1.loc[idx])
    idx = df2[df2['alpha'] == x]['reward'].idxmin()
    y2.append(df2.loc[idx])
    idx = df4[df4['alpha'] == x]['reward'].idxmin()
    y4.append(df4.loc[idx])
...
plt.plot(xs, y1['reward'])
# plt.fill_between(xs, y1['reward'] - y1['sem_reward'], y1['reward'] + y1['sem_reward'])
# plt.text(xs[0], y1['reward'][:1], r'$\lambda$ = 0')
plt.text(xs[-1], y1['reward'][-1:], r'$\lambda$ = 0')
# plt.text(2 ** (-4.8), 200, r'$\lambda$ = 0')
plt.plot(xs, y2['reward'])
# plt.fill_between(xs, y2['reward'] - y2['sem_reward'], y2['reward'] + y2['sem_reward'])
# plt.text(xs[0], y2['reward'][:1], r'$\lambda$ = 0.5')
plt.text(xs[-1], y2['reward'][-1:], r'$\lambda$ = 0.5')
# plt.text(2 ** (-5.9), 470, r'$\lambda$ = 0.5')
plt.plot(xs, y4['reward'])
# plt.fill_between(xs, y4['reward'] - y4['sem_reward'], y4['reward'] + y4['sem_reward'])
# plt.text(xs[0], y4['reward'][:1], r'$\lambda$ = 0.75')
plt.text(xs[-1], y4['reward'][-1:], r'$\lambda$ = 0.75')
# plt.text(2 ** (-7), 600, r'n = 4')

plt.title(r'REWARD vs $\alpha$')
plt.ylabel('REWARD')
plt.xlabel(r'$\alpha$')
# plt.legend()
plt.xscale('log', basex=2)
